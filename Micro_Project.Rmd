...
Our hypothesis for the first part is, that the mentioned variables have a significant effect on the average sentence length.

$$  avgsen = \beta_0 + \beta_1\ inc86 + \beta_2\ black +  \beta_3\ tottime +  \beta_4\ qemp86  $$ 

### Simple OLS-Estimation of the model

general OLS estimation including all potential regressors
```{r,include=TRUE}
...
# PART 2
We test significant variables on the probability of arrest in 1986. !!!Hypothesis
...
omitting insignificant variables:
...
Although OLS yiels unbiased estimators, heteroskedasticity among other things leads to inefficient ones. 
...
For comparison the probit model with same regressors
```{r}
...
```
slide 21 ... factor 1.6
The factor between our Probit and Logit is `factor_log_prob`
...
```{r}
library("oglmx")
results.olog<-oglmx(narr86 ~  0 + ptime86 + qemp86 + inc86  + black + hispan + pcnvsq + pt86sq + inc86sq, data=dfn, link="logit",
...
```{r}
"marginal effects"
margins.oglmx(results.olog,ascontinuous = TRUE) #treating discrete variables like continuous ones, gives reasonable approximations of marginal effects
```
...
results.ologalt<-oglmx(narr86 ~  0 + ptime86 + qemp86 + inc86  + black + hispan + pcnvsq + pt86sq + inc86sq, data=dfn, link="logit", threshparam=c(0.9,1.8,2.7))
