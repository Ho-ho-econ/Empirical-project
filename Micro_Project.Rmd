...
### Working on "avgsen"
The idea is to build a model estimating expected severity of conviction when arrested in 1986 using level of income, employment, total time spend in prison and color (black ad non-black) of the arrested. Hypothesis, which are to be tested, are that the coefficient of every single variable is equal to 0.
...
Some of the variables may be ednogenuos.
=> Testing  this way may not be correct.

#### IV Regression
The idea is to use instrumental variables in the estimation of the expected severity.
Define:
...
## parameter logit probit check, for "ptime86"
```{r}
1.251e+00/7.387e-01
```
slide 21 ... factor 1.6
